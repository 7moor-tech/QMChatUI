...
- (QMChatRoomXbotCardView*)cardView {
    if (!_cardView) {
        _cardView = [[QMChatRoomXbotCardView alloc] initWithFrame:CGRectMake(kChatLeftAndRightWidth,  CGRectGetMaxY(self.timeLabel.frame) + 25, QM_kScreenWidth - kChatLeftAndRightWidth * 2, 300)];
    }
    return _cardView;
}
...
- (void)setData:(CustomMessage *)message avater:(NSString *)avater {
    ...
    if(err) {
        ...
        CGFloat cellWidth = QM_kScreenWidth - kChatLeftAndRightWidth * 2;
        ...
        if (message.cardType == QMMessageCardTypeNone) {
            ...
            cellHeight = 88*listNumber + 72*shopNumber + 48;
            if (cardList.count > 0) {
                cellHeight += 40;
            }
        ...
        _cardView.frame = CGRectMake(kChatLeftAndRightWidth, CGRectGetMaxY(self.timeLabel.frame) + kChatTopMargin, cellWidth, cellHeight);
        _cardView.messageId = _messageId;
        _cardView.type = message.cardType;
        _cardView.showCount = showCount;
        _cardView.cardStyle = message.cardStyle;
